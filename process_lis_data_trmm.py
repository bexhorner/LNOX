...
def save_ncdf(lis_type, lats, lons, lis_rad_24h, lis_fl_24h, n_24h, lis_rad_hr, lis_fl_hr, n_hr, yr1, yr2):
    ...
    out_file = os.path.join( out_dir, 'test_trmm_lst_new.nc' )
...
if __name__ == "__main__":
    ...
    for yr in years:
        ...
        for i,f in enumerate(lis_files):
            ...
            for i in np.arange(len(fl_lat)):
                ...
                if utc_hr[i] < 0:
                    #print('Still negative UTC hour: ', utc_hr[i])
                    # Skip this iteration if UTC hour is still negative after correction
                    continue
                else:
                    tutc_hr = utc_hr[i]

                if utc_hr[i] > 24:
                    #print('UTC hour still too large: ', utc_hr[i])
                    # Skip this iteration if UTC hour is still negative after correction
                    continue
                else:
                    tutc_hr = utc_hr[i]
                ...
                if fl_radiance[i] == 0:
                    grid_24h_rad[lon_ind, lat_ind] += np.nan
                ...
                if fl_radiance[i] == 0:
                    grid_hr_rad[lon_ind,lat_ind,hr_ind] += np.nan
    ...
    iind,jind = np.where(grid_24h_count > 0)
    grid_24h_rad[iind,jind] = ( grid_24h_rad[iind,jind] /
                                grid_24h_count[iind,jind] )
    iind,jind,hind = np.where(grid_hr_count > 0)
